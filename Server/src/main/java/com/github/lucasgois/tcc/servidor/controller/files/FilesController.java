...
public class FilesController {
    ...
    public ResponseEntity<Object> create(@RequestBody @Valid final FilesDto body) throws SQLException {
        repository.create(body);
        final URI uri = URI.create(ENVIRONMENTS + "/" + body.getFile_hash());
        return ResponseEntity.created(uri).build();
    }

    @PutMapping(value = "{uuid}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Object> alterar(@PathVariable final String uuid, @RequestBody @Valid final FilesDto body) throws SQLException {
